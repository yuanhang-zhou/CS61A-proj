...
def sus_points(score):
    ...
    if factor_nums == 3 or factor_nums == 4:
        prime_score = score + 1
        while not is_prime(prime_score):
            prime_score += 1
        return prime_score
    else:
        return score
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def always_roll_helper(player_score, opponent_score):
        return n
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_score, max_score_roll_num = 0, 0
    for dice_num in range(1, 11):
        current_score = make_averaged(roll_dice, times_called)(dice_num, dice)
        if current_score > max_score:
            max_score = current_score
            max_score_roll_num = dice_num
