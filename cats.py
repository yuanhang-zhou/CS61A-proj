...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    return round(len(typed) * 60 / elapsed / 5, 2)
    # END PROBLEM 4
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if immutable_args in cache and cache[immutable_args][0] >= limit:
            return cache[immutable_args][1]
        else:
            result = diff_function(typed, source, limit)
            cache[immutable_args] = (limit, result)
            return result
        # END PROBLEM EC
    return memoized
...
def minimum_mewtations(typed, source, limit):
    ...
    """
    # assert False, 'Remove this line'
    if (limit < 0) or (typed != source and limit == 0): # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 0
        # END
...
def report_progress(typed, source, user_id, upload):
    ...
    while index < len(typed) and index < len(source):
        if typed[index] == source[index]:
            ...
            break
    user_progress = index / len(source)
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    res = [[] for p in player_indices]
    for w in word_indices:
        p, _ = min(((p, get_time(times, p, w)) for p in player_indices), key = lambda t: t[1])
        res[p].append(words[w])
    return res
