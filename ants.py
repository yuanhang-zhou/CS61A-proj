...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            occupy_ant = place.ant
            if self.can_contain(occupy_ant):
                self.store_ant(occupy_ant)
                place.ant = self
            elif occupy_ant.can_contain(self):
                occupy_ant.store_ant(self)
                place. ant = occupy_ant
            else:
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.is_container and self.ant_contained and not self.ant_contained.is_buffed:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        index = 0
        while place and not place.is_hive and index <= self.upper_bound:
            if place.bees and index >= self.lower_bound: 
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        total_damage = amount + (self.damage if amount >= self.health else 0)
